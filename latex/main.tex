...
This paper is a \textbf{critical survey} of evaluation frameworks for agentic AI systems, covering the research landscape from 2024 to early 2026. It does not introduce a new empirical benchmark or report original experimental results. Rather, it synthesizes findings across the literature, identifies structural gaps, and proposes a set of candidate conceptual frameworks to guide both researchers and practitioners. Specifically, our contributions are:

\begin{enumerate}
    \item A synthesis of the four-pillar evaluation model (core model, memory, tool use, environment), integrating findings across multiple foundational papers.
    \item A critical review of automated judge paradigms---LLM-as-a-Judge and Agent-as-a-Judge---including identified biases and the perturbation-based stress-testing methodology of the Judge Reliability Harness.
    \item A review and critical comparison of open-source and commercial evaluation tooling, including limitations and tradeoffs not typically discussed in vendor documentation.
    \item Two proposed conceptual frameworks for community discussion: the \textbf{CLEAR} deployment-readiness dimensions and \textbf{Cost-Normalized Accuracy (CNA)}. These are synthesis proposals, not empirically validated standards. Their constituent components draw on prior work; the synthesis and framing are this paper's contribution.
    \item Identification of the measurement imbalance in the literature and a concrete research agenda for closing it.
\end{enumerate}
...
% ══════════════════════════════════════════════════════════════════════
\input{sections/sec3_cognitive}
\input{sections/sec4_evaluator}
\input{sections/sec5_operational}
\input{sections/sec6_enterprise}
\input{sections/sec7_tooling}
\input{sections/sec89_future_conclusion}
...
Fei Wang is a co-founder of Alchedata AI, where he leads research on agentic AI systems and enterprise evaluation infrastructure. His work focuses on the intersection of LLM-based automation and production reliability. He is the corresponding author for this work.
\end{IEEEbiographynophoto}

\begin{IEEEbiographynophoto}{Salon Ren}
Salon Ren is a researcher at Alchedata AI specializing in multi-agent system design and evaluation methodology. His interests include trajectory-level observability and the operationalization of AI safety frameworks in production environments.
\end{IEEEbiographynophoto}
