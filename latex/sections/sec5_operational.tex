...
To illustrate how CNA reorders the conventional accuracy ranking, Table~\ref{tab:cna_results} presents a schematic comparison using representative architecture archetypes. The values are adapted from the evaluation setup described in~\citet{beyond_accuracy} and should be read as illustrative examples of the metric's behavior rather than as independently validated benchmark results. Figure~\ref{fig:cna_comparison} visualizes the same comparison.

\begin{table}[htbp]
\centering
\caption{Illustrative architecture comparison: efficacy, cost, and CNA (schematic example adapted from~\citet{beyond_accuracy}; values are not original experimental results).}
...
In this illustrative example, the Reflexion architecture---often cited for its high accuracy---is substantially more expensive than the domain-tuned alternative while delivering lower accuracy. The Domain-Tuned agent achieves the highest accuracy at the lowest cost, producing a CNA more than 15$\times$ higher than Reflexion. Real-world rankings will depend on specific task distributions, model versions, and pricing at evaluation time.

The broader concern this example illustrates is what researchers describe as a \keyword{distorted research landscape}, where expensive and fragile solutions appear superior in standard accuracy-only benchmarks~\citep{measurement_imbalance}. Simple baseline strategies can sometimes outperform complex agents at substantially lower cost---a pattern that accuracy-only reporting would obscure.

\textbf{We propose that CNA be adopted as a standard reporting companion to any accuracy metric.} Any benchmark result reported without cost normalization is incomplete from an economic standpoint. Whether CNA or a related cost-normalized metric becomes the community standard requires broader discussion, but the principle---that cost and accuracy must be reported together---is well-supported by the literature surveyed here.
